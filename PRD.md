...
- Purpose: An interactive, comedic audio-visual piece where ordinary cooking escalates into chaos. Users trigger most sounds via pointer actions; accidents trigger themselves as the story advances. Visuals are playful, candy-pastel abstractions. The story ends 5 seconds after the final Chaos cue with five full-canvas white blinks, a 3 s visual fade-out (background + grid remain), then a centered closing line appears.
...
- UI narrative is rendered as centered on-canvas text.
...
5.1 Per-state sound programming
- Preparing: `fridge`, `knife_sharpen`, `chopping`, `cutting_things`, `squeeze_juice`, `tap`, `bag_rustling`
- Cooking: `lighter`, `stove` (looped), `stove_fire`, `hissing_pan` (looped, requires `stoveOn`), `water_pour`, `glass_stirring`, `cooking_spray`, `microwave`, `glass_clink`, `boiling_water` (looped, requires `stoveOn`)
...
7) Visual system (palette, factories, mappings)
- Global palette (src/theme/palette.js):
  - background: #e8cb6d (vanilla yellow)
  - foreground: #1c1b13 (dark brown)
  - white: #f2f1df; skyBlue: #63f2cc; seaBlue: #5724d6; red: #db2b3a; green: #39c450; orange: #eb8016; lavendar: #ab2bc4
- Renderer background: Three.js scene background uses palette.background for high contrast.
- Shared visual API (factory output):
  - create(params) → { id, update(dt), setPosition(x,y), getPosition(), destroy(), object3D }
  - Visuals must source colors exclusively from the global palette.
- Audio-visual mappings:
  - fridge → fridgeDrop (randomized orientation on spawn; bounce to rest pool)
  - bag_rustling → bagRustleSides (spawn from top-left/right with inward velocity; random rotation)
  - lighter → blinkOverlay (full-canvas blink on gate; auto fade)
  - stove, stove_fire → redSpectrogram (steady red spectrum; fades after silence)
  - hissing_pan → redFilterOverlay (full-canvas red filter to opacity ~0.2)
  - water_pour → waterPourStream (continuous stream at pointer while active; clears juice stains)
  - glass_stirring → glassArcSpinner (white arcs around canvas center; each fades after ~3 revolutions)
  - cooking_spray → whiteSprayDroplets (fast white droplets; persist until `spray:clear`)
  - microwave → microwaveSineWaves (lavender thin horizontal sines moving in opposite directions; fades after silence)
  - glass_clink → glassClinkRings (large skyBlue rings on gate; persist until `clink:clear`)
  - boiling_water → boilingVerticalSine (fixed-parameter vertical white sines; upward motion via phase progression only)
  - tap → tapWaterSpectrogram (steady blue spectrum at bottom + water stream)
  - squeeze_juice → juiceDroplets (z-axis droplets toward canvas; center-biased impacts; persist until `juice:clear`)
  - fire_alarm → fireAlarmBlinkFrame (red frame around canvas; blinks on gate hits; fades after silence)
  - clatter → clatterFissure (foreground crack, thick connected segments; persists across hits)
  - pot_falling → potFallingShake (full-canvas shake via fx:shake on spawn and hits)
  - objects_falling → objectsFallingBounce (bouncy shapes that keep bouncing; wall/floor collisions; persist)
  - thump → thumpRipple (foreground ring at pointer; opacity 0.8; expands wide and fades over ~2 s)
- Coordinate mapping: pointer (0..1) normalized → pixel space; y is inverted so DOM top maps to screen top; orthographic camera uses pixel units.
- Text rendering: centered sprite text on canvas; font size scales with canvas (~6% of min dimension, clamped), opacity target ~0.9; re-centers and re-renders on resize.

7.1) Gate trigger mechanism
- Each playing sound registers an analyser tap; short-term RMS is computed with attack/release smoothing and hysteresis.
- Per-sound gate thresholds live in config/app.json under each sound's `gate` field (e.g., `{ "thresholdDb": -30, "attackMs": 15, "releaseMs": 150, "minIntervalMs": 90 }`).
- On threshold crossing (with debounce), a `gate:hit` event triggers associated visuals; continuous `gate:level` events expose current RMS for debugging. Some visuals listen for custom clear signals emitted elsewhere: `juice:clear`, `spray:clear`, `clink:clear`.
- Renderer listens for `fx:shake` (detail: `{ durationMs, intensity }`) to apply transient full-canvas shake.
...
8.1) Debugging aids
- Gate loudness overlay: a small top-left UI shows per-sound RMS and dB (hidden from a11y). Enabled when debug is true.
...
11) On-canvas prompts and timing
- Opening line (centered): “Click Anywhere to Start Cooking”.
- State enter lines (centered):
  - Preparing: “Maybe light something.”
  - Cooking: “Multitasking is easy. Add more. More.”
  - Accident Breakout: “It’s fine. Totally fine.”
  - Chaos: “Nailed it.”
- Event narration lines (centered): shown for cues that include `text` in the config.
- Timing: after each event cue is triggered, the corresponding line appears after ~2.5 s (delayed), with ~0.7 s fade-in to ~0.9 opacity.
- Style guidance: In Preparing/Cooking, lines hint at next actions; in Accident Breakout/Chaos, lines describe what just happened.

12) Error handling and edge cases
- If AudioContext fails to start, keep the centered on-canvas opening line visible and log errors to console; audio resumes on user interaction.
...
- Event types and payloads
  ...
  - gate:hit {soundId, rms}
  - gate:level {soundId, rms}
  - error:{code,msg}
- Include a monotonically increasing event id and session id; include current state in each log.

16) Remaining sounds without newly rewritten visuals
- All primary sounds now have updated visuals as listed in section 7. No pending rewrites at this time.
...
- Chaos ending: 5 seconds after the final Chaos cue executes, audio is cut; then the canvas blinks white 5× (≈200 ms each), all non-grid visuals fade out over ~3 s, and the centered closing line appears with ~1.5 s fade-in.
...
- Visual factories implemented (current): accidentBursts, flashingCircularSpectrogram, fridgeDrop, knifeStripe, chopStripe, cutStripe, bagRustleSides, blinkOverlay, redSpectrogram, redFilterOverlay, waterPourStream, glassArcSpinner, whiteSprayDroplets, tapWaterSpectrogram, juiceDroplets, microwaveSineWaves, glassClinkRings, boilingVerticalSine, clatterFissure, potFallingShake, objectsFallingBounce, thumpRipple, fireAlarmBlinkFrame.
